...
public class CotrTaskCompletedListenerTest {
    ...
    private static WorkflowCOTRTaskType buildWorkflowCOTRTaskType() {
        return WorkflowCOTRTaskType.WorkflowCOTRTaskTypeBuilder.aWorkflowCOTRTaskType()
    ...
    public void shouldRaiseTaskDeletedPublicEventAfterCompletionWhenDeletionReasonPresent() {
        processInstance = runtimeService.createProcessInstanceByKey(COTR_CUSTOM_WORK_FLOW)
                ...
                .setVariable(COTR_TASK_VARIABLE_JSON_STRING, objectToJsonObjectConverter.convert(buildWorkflowCOTRTaskType()).toString())
                .execute();
        doAssertion(true);
    }
    ...
    private void doAssertion(boolean isDeleted) {
        final Optional<Task> optionalTask = taskService.createTaskQuery().processInstanceId(processInstance.getId()).active().list().stream().filter(task -> task.getTaskDefinitionKey().equals("cotr_custom_activity")).findFirst();
        ...
        assertThat("cotr_custom_activity", is(delegateTask.getTaskDefinitionKey()));
